...
- 509 份授权 PDF，7237 页原始内容；全量扫描未将视频纳入题库。
- 12 个章节、438 个去重后的案例题组、1862 个小问、133 套可练习整卷试卷（空白卷保留为来源，不重复进入模考列表）。
- 发现并合并 81 组重复题；每道题保留全部来源文件与页码，并优先保留更完整的答案内容。

## 访问保护

站点增加了访问口令门禁，口令只以 SHA-256 校验值参与前端验证，不在仓库中保存明文。验证会话仅保存在当前浏览器标签页；由于 GitHub Pages 是公开静态托管，这属于防止随意打开的轻量门禁，不等同于服务端权限控制。
